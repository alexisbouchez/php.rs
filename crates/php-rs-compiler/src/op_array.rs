...
//!
//! The [`ZOpArray`] is the compiled form of a PHP script, function, method,
//! or closure. It contains the opcode sequence, literal pool, variable tables,
//! argument info, and exception handling metadata. Every compilation unit in PHP
//! produces one `ZOpArray`.
//!
...
pub struct ArgInfo {
    ...
    /// Parameter attributes: Vec<(attr_name, Vec<(arg_name_or_none, arg_value)>)>.
    pub attributes: Vec<(String, Vec<(Option<String>, String)>)>,
}
...
pub struct ClassPropertyInfo {
    ...
    pub default: Option<Literal>,
    pub is_static: bool,
    /// Modifier flags (public=0x01, protected=0x02, private=0x04, readonly=0x200, etc.)
    pub flags: u32,
    /// Type hint name (if any), e.g. "string", "int", "?string", "int|string".
    pub type_name: Option<String>,
    /// Property get hook function name (e.g. "ClassName::$prop::get"), if any.
    pub get_hook: Option<String>,
    /// Property set hook function name (e.g. "ClassName::$prop::set"), if any.
...
pub struct ClassMetadata {
    ...
    /// Per-method modifier flags: (method_name, flags).
    pub method_flags: Vec<(String, u32)>,
    /// Trait adaptations: precedence (insteadof) and alias (as).
    pub trait_adaptations: Vec<TraitAdaptationInfo>,
}
...
pub enum TraitAdaptationInfo {
    ...
    Alias {
        ...
        method: String,
        alias: Option<String>,
        /// Visibility flags (ACC_PUBLIC=0x01, ACC_PROTECTED=0x02, ACC_PRIVATE=0x04)
...
pub struct ZOpArray {
    ...
    /// Return type hint (if any), e.g. "int", "string", "?Foo", "int|string", "void", "never".
    ...
    /// Whether this op_array's file has `declare(strict_types=1)`.
    pub strict_types: bool,

    /// Tick count from `declare(ticks=N)`. 0 means no ticks.
    pub ticks: u32,

    /// Script encoding from `declare(encoding='...')`. None means default (UTF-8).
    pub encoding: Option<String>,

    /// Byte offset of data after `__halt_compiler();` (0 means not present).
    pub halt_compiler_offset: u64,

    /// Function/method attributes: Vec<(attr_name, Vec<(arg_name_or_none, arg_value)>)>.
    pub attributes: Vec<(String, Vec<(Option<String>, String)>)>,
}
...
pub enum Literal {
    ...
    /// Jump table for SWITCH_LONG: int value → opline offset
    LongJumpTable(HashMap<i64, u32>),
    /// Jump table for SWITCH_STRING: string value → opline offset
    StringJumpTable(HashMap<String, u32>),
}
...
impl Literal {
    ...
    pub fn as_php_string(&self) -> String {
        match self {
            ...
            Literal::LongJumpTable(map) => format!("LongJumpTable({} entries)", map.len()),
            Literal::StringJumpTable(map) => format!("StringJumpTable({} entries)", map.len()),
        }
    }
}
