...
//!
//! Walks the [`php_rs_parser`] AST and emits [`ZOp`] instructions into a
//! [`ZOpArray`]. This is the Rust equivalent of `php-src/Zend/zend_compile.c`,
//! handling all PHP 8.6 language constructs: expressions, control flow, OOP
//! declarations, closures, generators, match expressions, named arguments,
//! attributes, enums, and more.

use php_rs_parser::{
    Argument, ArrayElement, Attribute, BinaryOperator, CastType, ClassMember, ClosureUse,
    EnumMember, Expression, MagicConstantKind, Modifier, Name, Parameter, Program,
    PropertyHookBody, PropertyHookKind, Statement, TraitAdaptation, UnaryOperator, UseKind,
};
...
use crate::op_array::{
    ArgInfo, ClassMetadata, ClassPropertyInfo, Literal, TraitAdaptationInfo, ZOpArray,
};
use crate::opcode::ZOpcode;

/// Result of compiling an expression -- describes where the computed value lives.
///
/// After the compiler emits opcodes for an expression, the result value will be
/// in one of: a constant slot, a compiled variable (CV), or a temporary variable.
/// This struct captures the operand location and type so the caller can reference
/// the value in subsequent instructions.
#[derive(Debug, Clone, Copy)]
pub struct ExprResult {
    /// The operand referencing the value's location.
    pub operand: Operand,
    /// The type of the operand (Const, Cv, TmpVar, etc.).
...
/// The PHP compiler -- walks AST nodes and emits opcodes into a [`ZOpArray`].
///
/// The compiler performs a single pass over the AST, generating Zend VM instructions.
/// It handles:
/// - Expression compilation with correct operand allocation (constants, CVs, temps)
/// - Control flow (if/else, loops, switch/match, try/catch) with jump backpatching
/// - Function and class declarations (emitted as nested [`ZOpArray`] instances)
/// - Namespace resolution and `use` imports
/// - Closure variable capture and generator detection
/// - Label/goto resolution
///
/// # Usage
///
/// Prefer the free functions [`compile`], [`compile_file`], or [`compile_optimized`]
/// for typical use. Use the `Compiler` struct directly for advanced scenarios
/// (e.g., providing a custom filename).
pub struct Compiler {
    op_array: ZOpArray,
    ...
    /// Label map for goto: label_name → opline index.
    label_map: HashMap<String, u32>,
    /// Pending goto fixups: (opline_index, label_name) for forward gotos.
    goto_fixups: Vec<(u32, String)>,
}
...
impl Compiler {
    /// Create a new compiler with default settings (no filename, no namespace).
    pub fn new() -> Self {
        Self {
    ...
    /// Compile trait adaptations into ClassMetadata.
    fn compile_trait_adaptations(
        &self,
        ...
        for adaptation in adaptations {
            match adaptation {
                TraitAdaptation::Precedence {
                    ...
                    metadata
                        .trait_adaptations
                        .push(TraitAdaptationInfo::Precedence {
                            trait_name: trait_name.as_ref().map(|n| self.qualify_parsed_name(n)),
                            method: method.clone(),
                            insteadof: insteadof
                                .iter()
                                .map(|n| self.qualify_parsed_name(n))
                                .collect(),
                        });
                }
                TraitAdaptation::Alias {
                    ...
                    let visibility = modifiers.first().map(|m| modifier_flag(m));
                    metadata.trait_adaptations.push(TraitAdaptationInfo::Alias {
                        trait_name: trait_name.as_ref().map(|n| self.qualify_parsed_name(n)),
                        method: method.clone(),
                        alias: alias.clone(),
                        visibility,
    ...
    /// Compile a complete [`Program`] AST into a [`ZOpArray`].
    ///
    /// Consumes the compiler and returns the resulting opcode array. The op array
    /// includes an implicit `RETURN 1` at the end (PHP convention for successful
    /// script execution).
    pub fn compile_program(mut self, program: &Program) -> ZOpArray {
        ...
        // Resolve forward goto fixups
        for (opline, label) in &self.goto_fixups {
            if let Some(&target) = self.label_map.get(label) {
                self.op_array.opcodes[*opline as usize].op1.val = target;
            }
        }

        // Propagate strict_types to all nested function definitions (closures, methods, etc.)
        // In PHP, strict_types is per-file and affects all code in that file.
        if self.op_array.strict_types {
            Self::propagate_strict_types(&mut self.op_array);
        }

        self.op_array
    }

    /// Recursively propagate strict_types flag to nested function definitions.
    fn propagate_strict_types(op_array: &mut ZOpArray) {
        for func_def in &mut op_array.dynamic_func_defs {
            func_def.strict_types = true;
            Self::propagate_strict_types(func_def);
        }
    }
    ...
    fn compile_stmt(&mut self, stmt: &Statement) {
        match stmt {
            ...
            Statement::Return { value, span } => {
                ...
                let (ret_operand, ret_op_type) = if let Some(expr) = value {
                    let result = self.compile_expr(expr);
                    (result.operand, result.op_type)
                } else {
                    let null_idx = self.op_array.add_literal(Literal::Null);
                    (Operand::constant(null_idx), OperandType::Const)
                };

                // Emit VerifyReturnType / VerifyNeverType if function has a return type
                if let Some(ref rt) = self.op_array.return_type.clone() {
                    if rt == "never" {
                ...
                self.op_array
                    .emit(ZOp::new(ZOpcode::Return, line).with_op1(ret_operand, ret_op_type));
            }
            ...
            Statement::Foreach {
                ...
                self.compile_foreach(iterable, key, value, body, span, *by_ref);
            }
                ...
                for var in vars {
                    match var {
                        ...
                        Expression::StaticPropertyAccess {
                            ...
                            self.op_array.emit(
                                ZOp::new(ZOpcode::UnsetStaticProp, span.line as u32)
            ...
            Statement::Const {
                ...
                for (const_name, const_value) in consts {
                    ...
                    if attributes.is_empty() {
                        ...
                        // Encode attributes as "attr1\x02arg=val\x01attr2\x02..."
                        let mut attr_str = String::new();
                        for (i, attr) in attributes.iter().enumerate() {
                            if i > 0 {
                                attr_str.push('\x01');
                            }
                            let attr_name = self.qualify_name(&attr.name.parts.join("\\"));
                            attr_str.push_str(&attr_name);
                            for arg in &attr.args {
                                attr_str.push('\x02');
                                if let Some(ref name) = arg.name {
                                    attr_str.push_str(name);
                                    attr_str.push('=');
                                }
                                attr_str.push_str(&Self::attr_arg_to_string(&arg.value));
                            }
                        }
                        let attr_lit = self.op_array.add_literal(Literal::String(attr_str));
                        self.op_array.emit(
                            ZOp::new(ZOpcode::DeclareAttributedConst, span.line as u32)
                                .with_op1(Operand::constant(name_lit), OperandType::Const)
                                .with_op2(val.operand, val.op_type)
                                .with_extended_value(attr_lit),
                        );
                    }
                }
            }
            ...
            Statement::Static { vars, span } => {
                for sv in vars {
                    // Strip leading $ if present (parser stores raw source text)
                    let var_name = sv.name.strip_prefix('$').unwrap_or(&sv.name);
            ...
            Statement::Declare {
                ...
                for (name, value) in directives {
                    match name.as_str() {
                        "strict_types" => {
                            if let Expression::IntLiteral { value: n, .. } = value {
                                self.op_array.strict_types = *n == 1;
                            }
                        }
                        "ticks" => {
                            if let Expression::IntLiteral { value: n, .. } = value {
                                self.op_array.ticks = *n as u32;
                            }
                        }
                        "encoding" => {
                            if let Expression::StringLiteral { value: s, .. } = value {
                                self.op_array.encoding = Some(s.clone());
                            }
                        }
                        _ => {}
                    }
                }
                // Compile the body (for declare(ticks=N) { ... } form)
                if let Statement::Block { statements, .. } = body.as_ref() {
                    if !statements.is_empty() {
                        let ticks = self.op_array.ticks;
                        for s in statements {
                            self.compile_stmt(s);
                            // Emit Ticks opcode after each statement if ticks > 0
                            if ticks > 0 {
                                let tick_lit =
                                    self.op_array.add_literal(Literal::Long(ticks as i64));
                                self.op_array.emit(
                                    ZOp::new(ZOpcode::Ticks, 0)
                                        .with_op1(Operand::constant(tick_lit), OperandType::Const),
                                );
                            }
                        }
                    }
            ...
            Statement::Label { name, .. } => {
                let opline = self.op_array.next_opline();
                self.label_map.insert(name.clone(), opline);
            }
            ...
            Statement::Goto { label, span } => {
                ...
                if let Some(&target) = self.label_map.get(label) {
                    ...
                    // Forward goto — emit placeholder, fix up later
                    let opline = self.op_array.next_opline();
                    self.op_array.emit(
                        ZOp::new(ZOpcode::Jmp, line)
                            .with_op1(Operand::jmp_target(0), OperandType::Unused),
                    );
                    self.goto_fixups.push((opline, label.clone()));
                }
            }

            Statement::HaltCompiler { offset, .. } => {
                // Store the byte offset where data begins after __halt_compiler();
                self.op_array.halt_compiler_offset = *offset as u64;
                // Define __COMPILER_HALT_OFFSET__ constant
                let name_lit = self
                    .op_array
                    .add_literal(Literal::String("__COMPILER_HALT_OFFSET__".to_string()));
                let val_lit = self.op_array.add_literal(Literal::Long(*offset as i64));
                self.op_array.emit(
                    ZOp::new(ZOpcode::DeclareConst, 0)
                        .with_op1(Operand::constant(name_lit), OperandType::Const)
                        .with_op2(Operand::constant(val_lit), OperandType::Const),
                );
                // Emit Return to stop execution (no code after __halt_compiler)
                let null_lit = self.op_array.add_literal(Literal::Null);
                self.op_array.emit(
                    ZOp::new(ZOpcode::Return, 0)
                        .with_op1(Operand::constant(null_lit), OperandType::Const),
                );
            }
        }
    }
    ...
    fn compile_expr(&mut self, expr: &Expression) -> ExprResult {
        match expr {
            ...
            Expression::Variable { name, span } => {
                if name == "GLOBALS" {
                    // $GLOBALS is a special superglobal — emit FetchGlobals
                    let tmp = self.op_array.alloc_temp();
                    self.op_array.emit(
                        ZOp::new(ZOpcode::FetchGlobals, span.line as u32)
    ...
    fn compile_assign_ref(&mut self, lhs: &Expression, rhs: &Expression, line: u32) -> ExprResult {
        ...
        match lhs {
            Expression::PropertyAccess {
                ...
                // $obj->prop =& $var → AssignObjRef + OpData
                let obj = self.compile_expr(object);
                let prop = self.compile_expr(property);
                let tmp = self.op_array.alloc_temp();
                self.op_array.emit(
                    ZOp::new(ZOpcode::AssignObjRef, line)
                        .with_op1(obj.operand, obj.op_type)
                        .with_op2(prop.operand, prop.op_type)
                        .with_result(Operand::tmp_var(tmp), OperandType::TmpVar),
                );
            ...
            Expression::StaticPropertyAccess {
                ...
                // Class::$prop =& $var → AssignStaticPropRef + OpData
                let prop = self.compile_static_prop_name(property);
                let cls = self.compile_class_name_expr(class);
                let tmp = self.op_array.alloc_temp();
                self.op_array.emit(
                    ZOp::new(ZOpcode::AssignStaticPropRef, line)
                        .with_op1(prop.operand, prop.op_type)
                        .with_op2(cls.operand, cls.op_type)
                        .with_result(Operand::tmp_var(tmp), OperandType::TmpVar),
                );
            ...
            _ => {
                // Simple variable: $a =& $b → AssignRef (existing behavior)
                let lhs_result = self.compile_expr(lhs);
                let tmp = self.op_array.alloc_temp();
                self.op_array.emit(
    ...
    fn compile_binary_op(
        ...
        // Rope optimization: flatten chains of Concat into RopeInit/RopeAdd/RopeEnd
        if op == BinaryOperator::Concat {
            let mut parts = Vec::new();
            Self::collect_concat_parts(lhs, &mut parts);
            parts.push(rhs);
            if parts.len() >= 3 {
                return self.compile_rope(&parts, line);
            }
        }
    ...
    /// Recursively flatten a left-associative Concat chain into a list of parts.
    fn collect_concat_parts<'a>(expr: &'a Expression, parts: &mut Vec<&'a Expression>) {
        match expr {
            Expression::BinaryOp {
                ...
                Self::collect_concat_parts(lhs, parts);
                parts.push(rhs);
            }
            _ => parts.push(expr),
        }
    ...
    /// Compile a multi-part concat chain using RopeInit/RopeAdd/RopeEnd opcodes.
    fn compile_rope(&mut self, parts: &[&Expression], line: u32) -> ExprResult {
        let rope_tmp = self.op_array.alloc_temp();

        // RopeInit: first segment
        let first = self.compile_expr(parts[0]);
        self.op_array.emit(
            ZOp::new(ZOpcode::RopeInit, line)
                .with_op2(first.operand, first.op_type)
                .with_result(Operand::tmp_var(rope_tmp), OperandType::TmpVar)
                .with_extended_value(parts.len() as u32),
        );

        // RopeAdd: middle segments
        for (i, part) in parts[1..parts.len() - 1].iter().enumerate() {
            let part_result = self.compile_expr(part);
            self.op_array.emit(
                ZOp::new(ZOpcode::RopeAdd, line)
                    .with_op1(Operand::tmp_var(rope_tmp), OperandType::TmpVar)
                    .with_op2(part_result.operand, part_result.op_type)
                    .with_result(Operand::tmp_var(rope_tmp), OperandType::TmpVar)
                    .with_extended_value((i + 1) as u32),
            );
        }

        // RopeEnd: last segment — result goes to a new temp
        let result_tmp = self.op_array.alloc_temp();
        let last = self.compile_expr(parts[parts.len() - 1]);
        self.op_array.emit(
            ZOp::new(ZOpcode::RopeEnd, line)
                .with_op1(Operand::tmp_var(rope_tmp), OperandType::TmpVar)
                .with_op2(last.operand, last.op_type)
                .with_result(Operand::tmp_var(result_tmp), OperandType::TmpVar)
                .with_extended_value((parts.len() - 1) as u32),
        );
    ...
    fn compile_compound_assign(
        ...
        match lhs {
            ...
            Expression::ArrayAccess { array, index, .. } => {
                // $a[$k] op= $val → AssignDimOp + OpData
                let arr = self.compile_expr(array);
                let result_tmp = self.op_array.alloc_temp();
                let mut zop = ZOp::new(ZOpcode::AssignDimOp, line)
                    .with_op1(arr.operand, arr.op_type)
                    .with_result(Operand::tmp_var(result_tmp), OperandType::TmpVar)
                    .with_extended_value(assign_op as u8 as u32);
                if let Some(idx_expr) = index {
                    let idx = self.compile_expr(idx_expr);
                    zop = zop.with_op2(idx.operand, idx.op_type);
                }
                self.op_array.emit(zop);
                self.op_array.emit(
                    ZOp::new(ZOpcode::OpData, line)
                        .with_op1(rhs_result.operand, rhs_result.op_type),
                );
            ...
            Expression::PropertyAccess {
                ...
                // $obj->prop op= $val → AssignObjOp + OpData
                let obj = self.compile_expr(object);
                let prop = self.compile_expr(property);
                let result_tmp = self.op_array.alloc_temp();
                self.op_array.emit(
                    ZOp::new(ZOpcode::AssignObjOp, line)
                        .with_op1(obj.operand, obj.op_type)
                        .with_op2(prop.operand, prop.op_type)
                        .with_result(Operand::tmp_var(result_tmp), OperandType::TmpVar)
                        .with_extended_value(assign_op as u8 as u32),
            ...
            Expression::StaticPropertyAccess {
                ...
                // ClassName::$prop op= $val → AssignStaticPropOp + OpData
                let prop = self.compile_static_prop_name(property);
                let cls = self.compile_class_name_expr(class);
                let result_tmp = self.op_array.alloc_temp();
                self.op_array.emit(
                    ZOp::new(ZOpcode::AssignStaticPropOp, line)
                        .with_op1(prop.operand, prop.op_type)
                        .with_op2(cls.operand, cls.op_type)
                        .with_result(Operand::tmp_var(result_tmp), OperandType::TmpVar)
                        .with_extended_value(assign_op as u8 as u32),
    ...
    fn compile_unary_op(
        ...
        // ErrorSuppress needs BEGIN_SILENCE *before* the inner expression
        if op == UnaryOperator::ErrorSuppress {
            let silence_tmp = self.op_array.alloc_temp();
            self.op_array.emit(
                ZOp::new(ZOpcode::BeginSilence, line)
                    .with_result(Operand::tmp_var(silence_tmp), OperandType::TmpVar),
            );
            let inner = self.compile_expr(operand);
            self.op_array.emit(
                ZOp::new(ZOpcode::EndSilence, line)
                    .with_op1(Operand::tmp_var(silence_tmp), OperandType::TmpVar),
            );
            return inner;
        }
        ...
        match op {
            ...
            UnaryOperator::ErrorSuppress => {
                unreachable!("ErrorSuppress handled above before inner compilation");
            }
    ...
    fn compile_inc_dec(&mut self, opcode: ZOpcode, var: &Expression, line: u32) -> ExprResult {
        // Object property: ++$obj->prop / $obj->prop++
        if let Expression::PropertyAccess {
            object, property, ..
        } = var
        {
            let obj = self.compile_expr(object);
            let prop = self.compile_expr(property);
            let obj_opcode = match opcode {
                ZOpcode::PreInc => ZOpcode::PreIncObj,
                ZOpcode::PreDec => ZOpcode::PreDecObj,
                ZOpcode::PostInc => ZOpcode::PostIncObj,
                ZOpcode::PostDec => ZOpcode::PostDecObj,
                _ => unreachable!(),
            };
            let tmp = self.op_array.alloc_temp();
            self.op_array.emit(
                ZOp::new(obj_opcode, line)
                    .with_op1(obj.operand, obj.op_type)
                    .with_op2(prop.operand, prop.op_type)
                    .with_result(Operand::tmp_var(tmp), OperandType::TmpVar),
            );
        ...
        // Static property: ++Class::$prop / Class::$prop++
        if let Expression::StaticPropertyAccess {
            class, property, ..
        } = var
        {
            let cls = self.compile_class_name_expr(class);
            let prop = self.compile_static_prop_name(property);
            let static_opcode = match opcode {
                ZOpcode::PreInc => ZOpcode::PreIncStaticProp,
                ZOpcode::PreDec => ZOpcode::PreDecStaticProp,
                ZOpcode::PostInc => ZOpcode::PostIncStaticProp,
                ZOpcode::PostDec => ZOpcode::PostDecStaticProp,
                _ => unreachable!(),
            };
            let tmp = self.op_array.alloc_temp();
            self.op_array.emit(
                ZOp::new(static_opcode, line)
                    .with_op1(prop.operand, prop.op_type)
                    .with_op2(cls.operand, cls.op_type)
                    .with_result(Operand::tmp_var(tmp), OperandType::TmpVar),
            );
    ...
    fn compile_array_literal(&mut self, elements: &[ArrayElement], line: u32) -> ExprResult {
        ...
        for (i, elem) in elements.iter().enumerate() {
            ...
            if let Some(ref key_expr) = elem.key {
                ...
                // ...$spread — ensure array is initialized first
                if i == 0 {
                    self.op_array.emit(
                        ZOp::new(ZOpcode::InitArray, line)
                            .with_result(Operand::tmp_var(tmp), OperandType::TmpVar),
            ...
            match var {
                Expression::Variable { name, .. } if name == "this" => {
                    ...
                    compiler.op_array.emit(
                        ZOp::new(ZOpcode::IssetIsemptyThis, line)
                            .with_result(Operand::tmp_var(tmp), OperandType::TmpVar)
                ...
                Expression::PropertyAccess {
                    ...
                    let tmp = compiler.op_array.alloc_temp();
                    compiler.op_array.emit(
                        ZOp::new(ZOpcode::IssetIsemptyPropObj, line)
                            .with_op1(obj.operand, obj.op_type)
                            .with_op2(prop.operand, prop.op_type)
                            .with_result(Operand::tmp_var(tmp), OperandType::TmpVar)
                            .with_extended_value(0), // ISSET mode
                ...
                Expression::StaticPropertyAccess {
                    ...
                    let prop = compiler.compile_static_prop_name(property);
                    let cls = compiler.compile_class_name_expr(class);
                    let tmp = compiler.op_array.alloc_temp();
                    compiler.op_array.emit(
                        ZOp::new(ZOpcode::IssetIsemptyStaticProp, line)
                            .with_op1(prop.operand, prop.op_type)
                            .with_op2(cls.operand, cls.op_type)
                            .with_result(Operand::tmp_var(tmp), OperandType::TmpVar)
                            .with_extended_value(0), // ISSET mode
    ...
    fn compile_foreach(
        ...
        // Use RW variants for by-reference iteration
        let reset_opcode = if by_ref {
            ZOpcode::FeResetRw
        } else {
            ZOpcode::FeResetR
        };
        let fetch_opcode = if by_ref {
            ZOpcode::FeFetchRw
        } else {
            ZOpcode::FeFetchR
        };
    ...
    fn compile_switch(
        ...
        // Analyze case values to see if we can use optimized dispatch
        let all_int = cases.iter().all(|c| {
            c.condition.as_ref().map_or(true, |e| {
                matches!(e, Expression::IntLiteral { .. })
                    || matches!(e, Expression::UnaryOp { op, operand, .. }
                        if *op == UnaryOperator::Minus
                        && matches!(operand.as_ref(), Expression::IntLiteral { .. }))
            })
        });
        let all_string = cases.iter().all(|c| {
            c.condition
                .as_ref()
                .map_or(true, |e| matches!(e, Expression::StringLiteral { .. }))
        });
        let has_non_default = cases.iter().any(|c| c.condition.is_some());

        if has_non_default && (all_int || all_string) {
            self.compile_switch_optimized(condition, cases, line, all_int);
        } else {
            self.compile_switch_linear(condition, cases, line);
        }
    }

    /// Compile switch using optimized SwitchLong/SwitchString with a jump table.
    fn compile_switch_optimized(
        &mut self,
        condition: &Expression,
        cases: &[php_rs_parser::SwitchCase],
        line: u32,
        ...
        // Emit the SwitchLong/SwitchString opcode with a placeholder jump table.
        // op1 = condition, op2 = jump table literal (Const), extended_value = default target
        let jt_lit_idx = self.op_array.add_literal(Literal::Null); // placeholder
        let opcode = if is_long {
            ZOpcode::SwitchLong
        } else {
            ZOpcode::SwitchString
        };
        let switch_idx = self.op_array.emit(
            ZOp::new(opcode, line)
                .with_op1(cond.operand, cond.op_type)
                .with_op2(Operand::constant(jt_lit_idx), OperandType::Const),
        );

        // Compile each case body and record where each body starts.
        // We'll use this to build the jump table after all bodies are compiled.
        let mut case_targets: Vec<(Option<(bool, i64, String)>, u32)> = Vec::new(); // (case_value, body_start)
        ...
        for case in cases {
            ...
            if let Some(ref case_expr) = case.condition {
                let val = if is_long {
                    match case_expr {
                        Expression::IntLiteral { value, .. } => Some((true, *value, String::new())),
                        Expression::UnaryOp { operand, .. } => {
                            if let Expression::IntLiteral { value, .. } = operand.as_ref() {
                                Some((true, -*value, String::new()))
                            } else {
                                None
                            }
                        }
                    ...
                    match case_expr {
                        Expression::StringLiteral { value, .. } => Some((false, 0, value.clone())),
                        _ => None,
                    }
                };
                case_targets.push((val, body_start));
            } else {
                default_target = Some(body_start);
            }
        ...
        if is_long {
            ...
            for (val, target) in &case_targets {
                if let Some((true, int_val, _)) = val {
                    map.insert(*int_val, *target);
                }
            }
            self.op_array.literals[jt_lit_idx as usize] = Literal::LongJumpTable(map);
        } else {
            let mut map = HashMap::new();
            for (val, target) in &case_targets {
                if let Some((false, _, ref str_val)) = val {
                    map.insert(str_val.clone(), *target);
                }
            }
            self.op_array.literals[jt_lit_idx as usize] = Literal::StringJumpTable(map);
        }

        // Set the default target (or after_switch if no default)
        let default = default_target.unwrap_or(after_switch);
        self.op_array.opcodes[switch_idx as usize].extended_value = default;
    ...
    /// Compile switch using linear Case+Jmpz chain (fallback for mixed types).
    ...
    fn compile_function_decl(
        ...
        name: &str,
        params: &[Parameter],
        return_type: Option<&php_rs_parser::Type>,
        body: &[Statement],
        ...
        func_oa.attributes = self.encode_attributes(attributes);
            ...
            let param_attrs: Vec<(String, Vec<(Option<String>, String)>)> = param
                ...
                .map(|attr| {
                    ...
                    let args = attr
                        ...
                        .iter()
                        .map(|a| {
                            let val = Self::attr_arg_to_string(&a.value);
                            (a.name.clone(), val)
                        })
                        .collect();
                    (attr_name, args)
    ...
    fn compile_function_call(
        ...
        // Optimize known introspection functions to specialized opcodes
        if let Expression::StringLiteral { value, .. } = name {
            let lower = value.to_ascii_lowercase();
            match lower.as_str() {
                "gettype" if args.len() == 1 => {
                    let arg = self.compile_expr(&args[0].value);
                    let tmp = self.op_array.alloc_temp();
                    self.op_array.emit(
                        ZOp::new(ZOpcode::GetType, line)
                            .with_op1(arg.operand, arg.op_type)
                            .with_result(Operand::tmp_var(tmp), OperandType::TmpVar),
                    );
                    return ExprResult::tmp(tmp);
                }
                ...
                "get_called_class" if args.is_empty() => {
                    let tmp = self.op_array.alloc_temp();
                    self.op_array.emit(
                        ZOp::new(ZOpcode::GetCalledClass, line)
                            .with_result(Operand::tmp_var(tmp), OperandType::TmpVar),
                    );
                    return ExprResult::tmp(tmp);
                }
                "func_num_args" if args.is_empty() => {
                    let tmp = self.op_array.alloc_temp();
                    self.op_array.emit(
                        ZOp::new(ZOpcode::FuncNumArgs, line)
                            .with_result(Operand::tmp_var(tmp), OperandType::TmpVar),
                    );
                    return ExprResult::tmp(tmp);
                }
                "func_get_args" if args.is_empty() => {
                    let tmp = self.op_array.alloc_temp();
                    self.op_array.emit(
                        ZOp::new(ZOpcode::FuncGetArgs, line)
                            .with_result(Operand::tmp_var(tmp), OperandType::TmpVar),
                    );
                    return ExprResult::tmp(tmp);
                }
                "in_array" if args.len() == 2 || args.len() == 3 => {
                    // Determine strict flag at compile time if possible.
                    // extended_value: 0 = loose, 1 = strict
                    let strict_val = if args.len() == 3 {
                        match &args[2].value {
                            Expression::BoolLiteral { value: true, .. } => 1u32,
                            Expression::BoolLiteral { value: false, .. }
                            | Expression::Null { .. }
                            | Expression::IntLiteral { value: 0, .. } => 0u32,
                            _ => 0u32,
                        }
                    } else {
                        0u32
                    };
                    // Check if we should skip optimization (non-constant strict arg)
                    let skip = args.len() == 3
                        && !matches!(
                            &args[2].value,
                    ...
                    if !skip {
                        ...
                        let haystack = self.compile_expr(&args[1].value);
                        let tmp = self.op_array.alloc_temp();
                        self.op_array.emit(
                            ZOp::new(ZOpcode::InArray, line)
                                .with_op1(needle.operand, needle.op_type)
                                .with_op2(haystack.operand, haystack.op_type)
                                .with_result(Operand::tmp_var(tmp), OperandType::TmpVar)
                                .with_extended_value(strict_val),
                        );
                        return ExprResult::tmp(tmp);
                    }
                }
                "assert" if args.len() == 1 => {
                    // Emit AssertCheck before the assert call.
                    // If zend.assertions <= 0 at runtime, the VM jumps past the call.
                    let assert_check_idx = self.op_array.emit(ZOp::new(ZOpcode::AssertCheck, line));
                    // Compile assert as a normal function call
                    ...
                    // Patch AssertCheck to jump past DoFcall
                    let target = self.op_array.next_opline();
                    self.op_array.opcodes[assert_check_idx as usize].op2 =
                        Operand::jmp_target(target);
                    return ExprResult::tmp(tmp);
                }
                "array_key_exists" | "key_exists" if args.len() == 2 => {
                    let key = self.compile_expr(&args[0].value);
                    let arr = self.compile_expr(&args[1].value);
                    let tmp = self.op_array.alloc_temp();
                    self.op_array.emit(
                        ZOp::new(ZOpcode::ArrayKeyExists, line)
                            .with_op1(key.operand, key.op_type)
                            .with_op2(arr.operand, arr.op_type)
                            .with_result(Operand::tmp_var(tmp), OperandType::TmpVar),
                    );
                    return ExprResult::tmp(tmp);
                }
        ...
        for member in members {
            match member {
                ClassMember::Property {
                    ...
                    let mut set_hook_name = None;
                    for hook in hooks {
                        let hook_suffix = match hook.kind {
                            PropertyHookKind::Get => "get",
                            PropertyHookKind::Set => "set",
                        };
                        let hook_fn_name = format!("{}::${}::{}", fq_name, prop_name, hook_suffix);
                        let mut hook_oa = ZOpArray::for_function(&hook_fn_name);
                        hook_oa.line_start = hook.span.line as u32;

                        // Add $this as implicit CV
                        hook_oa.lookup_cv("this");

                        // For set hooks, add the value parameter
                        if hook.kind == PropertyHookKind::Set {
                            let param_name = if !hook.params.is_empty() {
                                let p = &hook.params[0];
                                p.name.strip_prefix('$').unwrap_or(&p.name).to_string()
                            } else {
                                "value".to_string()
                            };
                            hook_oa.lookup_cv(&param_name);
                            hook_oa.arg_info.push(ArgInfo {
                                name: param_name,
                            ...
                            hook_oa.required_num_args = 1;
                        }

                        let mut sub = self.sub_compiler(hook_oa);
                        match &hook.body {
                            PropertyHookBody::Expression(expr) => {
                                let val = sub.compile_expr(expr);
                                sub.op_array.emit(
                                    ZOp::new(ZOpcode::Return, line)
                                        .with_op1(val.operand, val.op_type),
                                );
                        ...
                        match hook.kind {
                            PropertyHookKind::Get => {
                                get_hook_name = Some(hook_fn_name);
                            }
                            PropertyHookKind::Set => {
                                set_hook_name = Some(hook_fn_name);
                            }
                        }
                        self.op_array.dynamic_func_defs.push(sub.op_array);
                    }
                    ...
                    if default_lit.is_some() || default.is_none() {
                        metadata.properties.push(ClassPropertyInfo {
                            ...
                            get_hook: get_hook_name,
                            set_hook: set_hook_name,
                        });
                    } else {
                        metadata.properties.push(ClassPropertyInfo {
                            name: prop_name.clone(),
                ...
                ClassMember::Method {
                    ...
                    // Encode method attributes
                    method_oa.attributes = self.encode_attributes(method_attrs);
                    ...
                    // Store method modifier flags
                    let meth_flags: u32 = method_mods.iter().map(modifier_flag).sum();
                    metadata
                        .method_flags
                        .push((method_name.clone(), meth_flags));
                ...
                ClassMember::TraitUse {
                    ...
                    self.compile_trait_adaptations(&mut metadata, adaptations);
                }
                // Abstract methods (no body) — create stub op_array for signature info
                ClassMember::Method {
                    name: method_name,
                    modifiers: method_mods,
                    body: None,
                    ...
                    let meth_flags: u32 = method_mods.iter().map(modifier_flag).sum();
                    // Mark as abstract
                    let meth_flags = meth_flags | 0x20; // ACC_ABSTRACT
                    metadata
                        .method_flags
                        .push((method_name.clone(), meth_flags));

                    // Create a stub op_array to store signature (arg_info + return_type)
                    let full_name = format!("{}::{}", fq_name, method_name);
                    let mut method_oa = ZOpArray::for_function(&full_name);
                    method_oa.line_start = line;
                    self.setup_params(&mut method_oa, params);
                    ...
                    self.op_array.dynamic_func_defs.push(method_oa);
                }
                _ => {}
            }
        }
        ...
        for member in members {
            match member {
                ClassMember::Method {
                    ...
                    let meth_flags: u32 = modifiers.iter().map(modifier_flag).sum();
                    metadata
                        .method_flags
                        .push((method_name.clone(), meth_flags));
                ...
                // Abstract interface methods (no body) — create stub op_array for signature
                ClassMember::Method {
                    name: method_name,
                    modifiers,
                    body: None,
                    ...
                    let base_flags: u32 = modifiers.iter().map(modifier_flag).sum();
                    let meth_flags: u32 = base_flags | 0x01 | 0x20;
                    metadata
                        .method_flags
                        .push((method_name.clone(), meth_flags));
        ...
        for member in members {
            match member {
                ClassMember::Method {
                    ...
                    // Store trait method modifier flags
                    let meth_flags: u32 = modifiers.iter().map(modifier_flag).sum();
                    metadata
                        .method_flags
                        .push((method_name.clone(), meth_flags));
                ...
                ClassMember::Property {
                    ...
                    let is_static = prop_mods.contains(&Modifier::Static);
                    let default_lit = default.as_ref().and_then(|e| Self::try_expr_to_literal(e));
                    let trait_prop_flags: u32 = prop_mods.iter().map(modifier_flag).sum();
                    metadata.properties.push(ClassPropertyInfo {
                        name: prop_name.clone(),
                        default: default_lit,
                        is_static,
                ...
                // Abstract methods in traits (no body)
                ClassMember::Method {
                    name: method_name,
                    modifiers: method_mods,
                    body: None,
    ...
    fn compile_enum_decl(
        ...
        for member in members {
            match member {
                ...
                EnumMember::ClassMember(ClassMember::TraitUse {
                    traits,
                    adaptations,
                    ..
                }) => {
    ...
    /// Encode AST attributes into the (attr_name, args) format used by op arrays.
    fn encode_attributes(
        &self,
        attributes: &[Attribute],
    ) -> Vec<(String, Vec<(Option<String>, String)>)> {
        attributes
            .iter()
            .map(|attr| {
                let attr_name = self.qualify_name(&attr.name.parts.join("\\"));
    ...
    /// Convert an attribute argument expression to a string representation.
    fn attr_arg_to_string(expr: &Expression) -> String {
        match expr {
            Expression::StringLiteral { value, .. } => value.clone(),
            ...
            Expression::ArrayLiteral { elements, .. } => {
                ...
                for (i, el) in elements.iter().enumerate() {
                    ...
                    if let Some(ref key) = el.key {
                        s.push_str(&Self::attr_arg_to_string(key));
                        s.push(':');
                    }
                    s.push_str(&Self::attr_arg_to_string(&el.value));
                }
                s.push(']');
                s
            }
...
fn modifier_flag(m: &Modifier) -> u32 {
    match m {
        ...
        // Asymmetric visibility (PHP 8.4): set-side visibility flags
        Modifier::PublicSet => 0x1000,
        Modifier::ProtectedSet => 0x2000,
        Modifier::PrivateSet => 0x4000,
    }
}
...
/// Apply basic optimization passes to a compiled [`ZOpArray`] in-place.
///
/// Currently performs:
/// - **Constant folding**: Evaluates constant binary operations at compile time
///   (e.g., `2 + 3` becomes `5`).
/// - **Dead code elimination**: Removes unreachable instructions after unconditional
///   jumps or returns.
///
/// Optimization is applied recursively to nested function/closure op arrays.
...
/// Compile a PHP source string to a [`ZOpArray`].
///
/// This is the primary entry point for compilation. It parses the source and
/// compiles it in one step. Returns a [`ParseError`](php_rs_parser::ParseError)
...
/// Compile a PHP source string with a known filename.
///
/// Same as [`compile`], but records the filename in the resulting [`ZOpArray`]
/// so that `__FILE__` and `__DIR__` magic constants resolve correctly at runtime.
...
/// Compile a PHP source string to an optimized [`ZOpArray`].
///
/// Equivalent to calling [`compile`] followed by [`optimize`]. Applies constant
/// folding and dead code elimination to the resulting opcode array.
...
mod tests {
    ...
    fn test_compile_switch() {
        // All-integer cases emit SwitchLong (optimized)
        let oa =
            compile_php("<?php switch ($x) { case 1: echo 'a'; break; case 2: echo 'b'; break; }");
        let ops = opcodes(&oa);
        assert!(
            ops.contains(&ZOpcode::SwitchLong),
            "All-int switch should emit SwitchLong"
        );
        assert!(ops.contains(&ZOpcode::Echo));

        // Mixed case types fall back to linear Case+Jmpz chain
        let oa2 = compile_php(
            "<?php switch ($x) { case 1: echo 'a'; break; case 'b': echo 'b'; break; }",
        );
        let ops2 = opcodes(&oa2);
        assert!(
            ops2.contains(&ZOpcode::Case),
            "Mixed switch should emit Case"
        );
    }
