//! ZOp -- a single Zend VM instruction.
//!
//! Defines [`ZOp`] (the instruction), [`Operand`] (the operand value), and
//! [`OperandType`] (how to interpret the operand). These mirror
//! `struct _zend_op`, `znode_op`, and the operand type constants from
...
///
/// Matches PHP's `IS_UNUSED`, `IS_CONST`, `IS_TMP_VAR`, `IS_VAR`, and `IS_CV`
...
pub enum OperandType {
    ...
    Unused = 0,
    /// Compile-time constant -- index into the `ZOpArray::literals` table.
    Const = 1, // 1 << 0
    /// Temporary variable -- intermediate computation result, not reference-counted.
    TmpVar = 2, // 1 << 1
    /// Engine-internal variable -- may be reference-counted (used for complex expressions).
    Var = 4, // 1 << 2
    /// Compiled variable -- a named local variable (e.g., `$x`), index into `ZOpArray::vars`.
    Cv = 8, // 1 << 3
}

/// An operand value -- a `u32` whose meaning depends on the associated [`OperandType`].
///
/// Interpretation by type:
/// - [`OperandType::Const`]: index into the `ZOpArray::literals` table
/// - [`OperandType::TmpVar`] / [`OperandType::Var`]: slot index in the temporary variable area
/// - [`OperandType::Cv`]: index into the `ZOpArray::vars` table (compiled variable table)
...
/// A single Zend VM instruction -- the fundamental unit of PHP bytecode.
///
/// Each instruction has an opcode, up to three operands (op1, op2, result),
/// an extended value for sub-operations, and a source line number.
///
