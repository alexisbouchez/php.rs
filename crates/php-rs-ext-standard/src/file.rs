...
enum StreamInner {
    ...
    Stdout,
    Stderr,
    /// In-memory buffer (for php://input, php://memory, php://temp).
    Memory(std::io::Cursor<Vec<u8>>),
    /// TCP client stream.
    TcpClient(TcpStream),
    /// TCP server listener.
    TcpServer(TcpListener),
    /// Readable pipe from a child process (stdout).
    ProcessStdout(std::process::ChildStdout),
    /// Readable pipe from a child process (stderr).
    ProcessStderr(std::process::ChildStderr),
    /// Writable pipe to a child process (stdin).
...
pub struct FileHandle {
    inner: StreamInner,
    eof: bool,
    /// Read filters attached to this stream (applied in order on read).
    pub read_filters: Vec<String>,
    /// Write filters attached to this stream (applied in order on write).
    pub write_filters: Vec<String>,
    /// Whether the stream is in blocking mode (default: true).
    pub blocking: bool,
...
impl FileHandle {
    ...
    /// Create an in-memory stream from raw bytes (used for php://input etc.)
    pub fn from_bytes(data: Vec<u8>) -> Self {
        Self {
            inner: StreamInner::Memory(std::io::Cursor::new(data)),
            eof: false,
            read_filters: Vec::new(),
            write_filters: Vec::new(),
            blocking: true,
    ...
    /// Create a handle wrapping a TCP client stream.
    pub fn from_tcp_stream(stream: TcpStream) -> Self {
        Self {
            inner: StreamInner::TcpClient(stream),
            eof: false,
            read_filters: Vec::new(),
            write_filters: Vec::new(),
            blocking: true,
    ...
    /// Create a handle wrapping a TCP server listener.
    pub fn from_tcp_listener(listener: TcpListener) -> Self {
        Self {
            inner: StreamInner::TcpServer(listener),
            eof: false,
            read_filters: Vec::new(),
            write_filters: Vec::new(),
            blocking: true,
    ...
    /// Accept a connection on a server socket, returning a new FileHandle for the client.
    pub fn accept(&self) -> io::Result<(Self, String)> {
        match &self.inner {
            StreamInner::TcpServer(listener) => {
                let (stream, addr) = listener.accept()?;
                Ok((Self::from_tcp_stream(stream), addr.to_string()))
            }
            _ => Err(io::Error::new(
    ...
    pub fn socket_name(&self, want_peer: bool) -> io::Result<String> {
        match &self.inner {
            StreamInner::TcpClient(s) => {
                if want_peer {
                    Ok(s.peer_addr()?.to_string())
                } else {
                    Ok(s.local_addr()?.to_string())
                }
            }
            StreamInner::TcpServer(l) => Ok(l.local_addr()?.to_string()),
            _ => Err(io::Error::new(
                io::ErrorKind::Unsupported,
                "Not a socket stream",
            )),
    ...
    pub fn set_blocking(&mut self, blocking: bool) -> io::Result<()> {
        match &self.inner {
            StreamInner::TcpClient(s) => s.set_nonblocking(!blocking)?,
            StreamInner::TcpServer(l) => l.set_nonblocking(!blocking)?,
            _ => {} // Silently succeed for non-socket streams (PHP behavior)
        }
    ...
    pub fn set_timeout(&mut self, timeout: Duration) -> io::Result<()> {
        ...
        match &self.inner {
            ...
            _ => {} // Silently succeed for non-socket streams
        }
        self.timeout = t;
        Ok(())
    }

    /// Check if the stream is a TCP socket (for stream_select).
    pub fn is_tcp(&self) -> bool {
        matches!(
            self.inner,
            StreamInner::TcpClient(_) | StreamInner::TcpServer(_)
        )
    }

    /// Try to clone the underlying TcpStream (for stream_select polling).
    pub fn try_clone_tcp(&self) -> Option<TcpStream> {
        match &self.inner {
            StreamInner::TcpClient(s) => s.try_clone().ok(),
            _ => None,
        }
    }

    /// Create a handle wrapping a child process stdin (writable).
    pub fn from_process_stdin(stdin: std::process::ChildStdin) -> Self {
        Self {
            inner: StreamInner::ProcessStdin(stdin),
            eof: false,
            read_filters: Vec::new(),
            write_filters: Vec::new(),
            blocking: true,
    ...
    /// Create a handle wrapping a child process stdout (readable).
    pub fn from_process_stdout(stdout: std::process::ChildStdout) -> Self {
        Self {
            inner: StreamInner::ProcessStdout(stdout),
            eof: false,
            read_filters: Vec::new(),
            write_filters: Vec::new(),
            blocking: true,
    ...
    /// Create a handle wrapping a child process stderr (readable).
    pub fn from_process_stderr(stderr: std::process::ChildStderr) -> Self {
        Self {
            inner: StreamInner::ProcessStderr(stderr),
            eof: false,
            read_filters: Vec::new(),
            write_filters: Vec::new(),
            blocking: true,
    ...
    pub fn read(&mut self, length: usize) -> io::Result<Vec<u8>> {
        ...
        let n = match &mut self.inner {
            ...
            StreamInner::Memory(c) => c.read(&mut buf)?,
            StreamInner::TcpClient(s) => s.read(&mut buf)?,
            StreamInner::ProcessStdout(stdout) => stdout.read(&mut buf)?,
            StreamInner::ProcessStderr(stderr) => stderr.read(&mut buf)?,
            StreamInner::Stdout
            | StreamInner::Stderr
            | StreamInner::TcpServer(_)
            | StreamInner::ProcessStdin(_) => {
                return Err(io::Error::new(
                    io::ErrorKind::Unsupported,
    ...
    pub fn write(&mut self, data: &[u8]) -> io::Result<usize> {
        match &mut self.inner {
            ...
            StreamInner::Memory(c) => c.write(data),
            StreamInner::TcpClient(s) => s.write(data),
            StreamInner::ProcessStdin(stdin) => stdin.write(data),
            StreamInner::Stdin
            | StreamInner::TcpServer(_)
            | StreamInner::ProcessStdout(_)
            | StreamInner::ProcessStderr(_) => Err(io::Error::new(
                io::ErrorKind::Unsupported,
    ...
    pub fn gets(&mut self) -> io::Result<Option<String>> {
        match &mut self.inner {
            ...
            StreamInner::Memory(c) => {
                use io::BufRead;
                let mut line = String::new();
                let n = c.read_line(&mut line)?;
                if n == 0 {
                    self.eof = true;
                    return Ok(None);
                }
            ...
            | StreamInner::ProcessStderr(_) => Err(io::Error::new(
                io::ErrorKind::Unsupported,
                "Cannot read line from this stream",
            )),
        }
    }
    ...
    pub fn seek(&mut self, offset: i64, whence: SeekWhence) -> io::Result<()> {
        ...
        match &mut self.inner {
            ...
            | StreamInner::Stdout
            | StreamInner::Stderr
            | StreamInner::TcpClient(_)
            | StreamInner::TcpServer(_)
            | StreamInner::ProcessStdin(_)
            | StreamInner::ProcessStdout(_)
            | StreamInner::ProcessStderr(_) => {
                return Err(io::Error::new(
                    io::ErrorKind::Unsupported,
    ...
    pub fn flush(&mut self) -> io::Result<()> {
        match &mut self.inner {
            ...
            StreamInner::TcpClient(s) => s.flush(),
            StreamInner::ProcessStdin(stdin) => stdin.flush(),
            StreamInner::Stdin
            | StreamInner::Memory(_)
            | StreamInner::TcpServer(_)
            | StreamInner::ProcessStdout(_)
            | StreamInner::ProcessStderr(_) => Ok(()),
        }
    }
