...
pub enum SerializableValue {
    ...
    /// Object: (class_name, properties as key-value pairs)
    Object(String, Vec<(SerializableValue, SerializableValue)>),
}
...
pub fn php_serialize(val: &SerializableValue) -> String {
    match val {
        ...
        SerializableValue::Object(class_name, props) => {
...
fn unserialize_value(data: &[u8], pos: usize) -> Option<(SerializableValue, usize)> {
    ...
    match data[pos] {
        ...
        b'O' => {
            // O:LEN:"CLASSNAME":COUNT:{key;value;...}
            if pos + 1 < data.len() && data[pos + 1] == b':' {
                let start = pos + 2;
                let colon1 = memchr_byte(b':', data, start)?;
                let len_str = std::str::from_utf8(&data[start..colon1]).ok()?;
                let len: usize = len_str.parse().ok()?;
                // Expect :"CLASSNAME":
                if colon1 + 1 >= data.len() || data[colon1 + 1] != b'"' {
                    return None;
                }
                let name_start = colon1 + 2;
                ...
                let class_name = std::str::from_utf8(&data[name_start..name_end])
                    .ok()?
                    .to_string();
                let count_start = name_end + 2;
                let colon2 = memchr_byte(b':', data, count_start)?;
                let count_str = std::str::from_utf8(&data[count_start..colon2]).ok()?;
                let count: usize = count_str.parse().ok()?;
                if colon2 + 1 >= data.len() || data[colon2 + 1] != b'{' {
                    return None;
                }
                let mut cur = colon2 + 2;
                ...
                Some((SerializableValue::Object(class_name, props), cur + 1))
            } else {
                None
            }
        }
