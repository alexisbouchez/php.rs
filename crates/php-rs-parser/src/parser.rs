//! Recursive descent / Pratt parser for PHP.
//!
//! Implements a complete PHP 8.6 parser with correct operator precedence using
//! Pratt parsing for expressions and recursive descent for statements. This is
//! equivalent to the Bison grammar in `php-src/Zend/zend_language_parser.y`.

use crate::ast::*;
use php_rs_lexer::{Lexer, Span, Token};

/// Parser for PHP source code -- converts tokens into an AST.
///
/// The parser consumes tokens from an internal [`Lexer`] and produces a
/// [`Program`] AST via the [`parse`](Parser::parse) method. Comments and
/// whitespace tokens are automatically skipped.
///
/// # Expression parsing
///
/// Expressions are parsed using a Pratt parser (top-down operator precedence)
/// that correctly handles PHP's operator precedence and associativity rules,
/// including ternary, null-coalesce, assignment, and all arithmetic/comparison
/// operators.
///
/// # Error handling
///
/// Parse errors are returned as [`ParseError`] with source location information.
/// The parser does not attempt error recovery -- it stops at the first error.
pub struct Parser<'a> {
    lexer: Lexer<'a>,
    current_token: Token,
    current_span: Span,
...
impl<'a> Parser<'a> {
    ...
    /// Parse a complete PHP program, returning a [`Program`] AST with all top-level statements.
    ///
    /// This is the main entry point. It handles the opening `<?php` tag, parses
    /// all statements until end-of-file, and returns the complete AST or the
    ...
    ///
    /// The `min_precedence` parameter controls which operators are consumed at this
    /// level. Pass `0` to parse a full expression. Higher values restrict parsing
    /// to tighter-binding operators only.
    ...
    /// Parse a single PHP statement (if, while, function, class, expression, etc.).
    ///
    /// Dispatches on the current token to the appropriate statement parser.
    /// Returns a [`Statement`] AST node or a [`ParseError`].
    pub fn parse_statement(&mut self) -> Result<Statement, ParseError> {
        ...
        match self.current_token {
            ...
            Token::Const => self.parse_const_statement_with_attributes(attributes),
            ...
            Token::HaltCompiler => {
                ...
                // Capture byte offset where data begins (lexer position after `;`)
                let offset = self.lexer.position();
                Ok(Statement::HaltCompiler {
    ...
    fn parse_const_statement(&mut self) -> Result<Statement, ParseError> {
        self.parse_const_statement_with_attributes(Vec::new())
    }

    fn parse_const_statement_with_attributes(
        &mut self,
    ...
    fn parse_class_member_with_attributes(
        ...
        loop {
            match self.current_token {
                ...
                Token::PublicSet => {
                    modifiers.push(Modifier::PublicSet);
                    self.advance();
                }
                Token::ProtectedSet => {
                    modifiers.push(Modifier::ProtectedSet);
                    self.advance();
                }
                Token::PrivateSet => {
                    modifiers.push(Modifier::PrivateSet);
                    self.advance();
                }
                Token::Static => {
    ...
    /// Parse a single trait adaptation statement (insteadof or as).
    ///
    /// Forms:
    ///   TraitName::method insteadof OtherTrait [, OtherTrait2];
    ///   method as newName;
    ///   method as visibility;
    ///   method as visibility newName;
    ///   TraitName::method as newName;
    ///   TraitName::method as visibility;
    ///   TraitName::method as visibility newName;
    fn parse_trait_adaptation(&mut self) -> Result<TraitAdaptation, ParseError> {
        // Parse: [TraitName::] method
        // First, get the identifier (could be a trait name or a method name)
        if !self.is_identifier_or_keyword() {
            return Err(self.error("identifier"));
        }
        let first_name = self.lexer.source_text(&self.current_span).to_string();
        self.advance();

        let (trait_name, method) = if self.current_token == Token::PaamayimNekudotayim {
            // TraitName::method
            self.advance();
            if !self.is_identifier_or_keyword() {
                return Err(self.error("method name"));
            }
            let method = self.lexer.source_text(&self.current_span).to_string();
            self.advance();
            (
                Some(Name {
                    parts: vec![first_name],
        ...
        if self.current_token == Token::Insteadof {
            // Precedence: method insteadof Trait1 [, Trait2, ...]
            self.advance();
            let mut insteadof = Vec::new();
            loop {
                insteadof.push(self.parse_qualified_name()?);
                if self.current_token == Token::Comma {
                    self.advance();
                    continue;
                }
            ...
            self.expect_semicolon()?;
            Ok(TraitAdaptation::Precedence {
                trait_name,
                method,
                insteadof,
            })
        } else if self.current_token == Token::As {
            // Alias: method as [visibility] [newName]
            self.advance();
            let mut modifiers = Vec::new();
            let mut alias = None;
            ...
            // Check for alias name (identifier after optional visibility)
            if self.is_identifier_or_keyword() {
                alias = Some(self.lexer.source_text(&self.current_span).to_string());
                self.advance();
            }
    ...
    fn parse_single_type(&mut self) -> Result<Type, ParseError> {
        ...
        // Handle DNF parenthesized group: (A&B)
        if self.current_token == Token::LParen {
            ...
            while self.current_token == Token::Ampersand {
                self.advance();
                group.push(self.parse_single_type()?);
            }
            self.expect(Token::RParen)?;
            // Return as Intersection if standalone, or let caller build DNF
            if group.len() == 1 {
    ...
    fn parse_prefix(&mut self) -> Result<Expression, ParseError> {
        ...
        match token {
            ...
            // Backtick string — equivalent to shell_exec()
            Token::BacktickString => {
                let text = self.lexer.source_text(&span);
                // Wrap in double quotes for parse_string_literal to process escape sequences
                let as_dq = format!(
                    "\"{}\"",
                    if text.len() >= 2 {
                        &text[1..text.len() - 1]
                ...
                let value = parse_string_literal(&as_dq);
                self.advance();
                // Convert to shell_exec($value) function call
                Ok(Expression::FunctionCall {
                    name: Box::new(Expression::StringLiteral {
                        value: "shell_exec".to_string(),
                        span,
                    }),
                    args: vec![Argument {
                        value: Expression::StringLiteral { value, span },
                        name: None,
                        unpack: false,
                        by_ref: false,
                    }],
...
///
/// Error messages follow PHP's style:
/// `Parse error: syntax error, unexpected token "}", expecting ";" on line 5`
///
/// Use [`ParseError::line`] and [`ParseError::column`] to extract the source
/// position for diagnostic display.
#[derive(Debug, Clone, PartialEq)]
pub enum ParseError {
    /// An unexpected token was encountered while parsing.
    UnexpectedToken {
        /// Description of what was expected (e.g., `";"`, `"expression"`).
        expected: String,
