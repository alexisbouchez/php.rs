...
//!
//! This module defines the complete set of AST nodes for PHP 8.6, matching the
//! grammar productions from `php-src/Zend/zend_language_parser.y`. Every node
//! carries a [`Span`] for source location tracking.
//!
//! The AST is structured as:
//! - [`Program`] -- root node containing a list of statements
//! - [`Statement`] -- control flow, declarations, blocks
//! - [`Expression`] -- values, operations, calls
//! - Supporting types: [`Name`], [`Parameter`], [`ClassMember`], [`Type`], etc.

use php_rs_lexer::Span;

/// Root AST node representing a complete PHP program (file).
///
...
/// A PHP statement -- represents all statement-level constructs.
///
/// Statements are the building blocks of PHP programs: control flow (`if`, `while`,
/// `for`), declarations (`function`, `class`, `namespace`), expression statements
/// (`$x = 1;`), and structural elements (`try/catch`, `switch`).
///
...
pub enum Statement {
    ...
    HaltCompiler {
        remaining: String,
        /// Byte offset of data after `__halt_compiler();` in the source.
        offset: usize,
        span: Span,
    },
}

/// A PHP expression -- represents all expression-level constructs.
///
/// Expressions are values or computations that produce values: literals,
/// variable references, binary/unary operations, function calls, property
/// access, array literals, casts, `new`, `clone`, `instanceof`, closures, etc.
///
/// Operator precedence is resolved during parsing using a Pratt parser,
/// so the AST directly represents the correct evaluation order.
///
/// Every variant includes a [`Span`] field for source location.
#[derive(Debug, Clone, PartialEq)]
pub enum Expression {
...
pub enum Modifier {
    ...
    Final,
    Readonly,
    /// `public(set)` — readable publicly, writable only with public access (PHP 8.4)
    PublicSet,
    /// `protected(set)` — readable publicly, writable only from class hierarchy (PHP 8.4)
    ProtectedSet,
    /// `private(set)` — readable publicly, writable only from declaring class (PHP 8.4)
    PrivateSet,
}
