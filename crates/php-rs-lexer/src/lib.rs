//! PHP lexer/tokenizer -- lexical analysis for PHP source code.
//!
//! This crate implements the lexical scanner that converts raw PHP source text
//! into a stream of [`Token`]/[`Span`] pairs. It is equivalent to the hand-written
//! re2c scanner in `php-src/Zend/zend_language_scanner.l`.
//!
//! # Main types
//!
//! - [`Lexer`] -- The tokenizer state machine. Accepts a `&str` of source code
//!   and produces `(Token, Span)` pairs via [`Lexer::next_token`].
//! - [`Token`] -- Enum of all PHP token types (keywords, operators, literals,
//!   comments, PHP tags, etc.), matching the `T_*` constants from php-src.
//! - [`Span`] -- Source location with byte offsets and line/column numbers,
//!   used for error messages and AST node positions.
//!
//! # Usage
//!
//! ```rust,ignore
//! use php_rs_lexer::{Lexer, Token};
//!
//! let mut lexer = Lexer::new("<?php echo 42;");
//! while let Some((token, span)) = lexer.next_token() {
//!     if token == Token::End { break; }
//!     println!("{:?} at line {}", token, span.line);
//! }
//! ```
