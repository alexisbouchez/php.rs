...
//!
//! Every variant corresponds to a `T_*` constant from the PHP tokenizer.
//! Reference: `php-src/Zend/zend_language_parser.y`

/// Represents a single token type produced by the PHP lexer.
///
/// This enum covers the full set of tokens defined in
/// `php-src/Zend/zend_language_parser.y`, including:
///
/// - **Literals**: integer, float, string, variable, inline HTML
/// - **Keywords**: all PHP reserved words (`if`, `class`, `function`, `echo`, etc.)
/// - **Operators**: arithmetic, comparison, assignment, bitwise, logical, etc.
/// - **Cast operators**: `(int)`, `(string)`, `(array)`, etc.
/// - **PHP tags**: `<?php`, `<?=`, `?>`
/// - **String interpolation**: `${`, `{$`, heredoc/nowdoc markers
/// - **Magic constants**: `__LINE__`, `__FILE__`, `__CLASS__`, etc.
/// - **Special tokens**: `End` (EOF), `Error`, `BadCharacter`
///
/// Each variant's doc comment includes the corresponding `T_*` constant name
/// from php-src for cross-reference.
...
pub enum Token {
    ...
    StringVarname,
    /// Backtick string for shell execution (equivalent to shell_exec())
    BacktickString,
...
impl std::fmt::Display for Token {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let s = match self {
            ...
            Token::BacktickString => "backtick string",
