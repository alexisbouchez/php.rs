...
//!
//! Provides the [`Span`] type for tracking the precise location of tokens
//! and AST nodes within PHP source files. Used throughout the lexer, parser,
//! and compiler for error reporting and source mapping.

/// A source location span -- tracks where a token or AST node appears in the source.
///
/// Each `Span` stores both **byte offsets** (for efficient `&str` slicing) and
/// **human-readable** line/column numbers (for error messages). Byte offsets use
/// a half-open range: `[start, end)`.
///
/// # Example
///
/// ```rust,ignore
/// let span = Span::new(6, 10, 1, 7); // "echo" at line 1, column 7
