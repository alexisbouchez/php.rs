...
//!
//! Hand-written state-machine lexer equivalent to the re2c-generated scanner in
//! `php-src/Zend/zend_language_scanner.l`. Supports all PHP 8.6 token types
//! including string interpolation, heredoc/nowdoc, and `__halt_compiler`.

use crate::{Span, Token};

/// Lexer state machine states, mirroring php-src's scanner states.
...
/// PHP lexer -- tokenizes PHP source code into a stream of [`Token`]/[`Span`] pairs.
///
/// The lexer operates as a state machine with five states (initial HTML, scripting,
/// double-quoted string, heredoc, and nowdoc) to correctly handle PHP's complex
/// tokenization rules including string interpolation and mixed HTML/PHP content.
///
/// # Usage
///
/// ```rust,ignore
/// let mut lexer = Lexer::new("<?php echo 42;");
...
/// # Features
///
/// - Full PHP 8.6 token support
/// - String interpolation (`"Hello $name"`, `"Hello {$obj->name}"`)
/// - Heredoc and nowdoc syntax
/// - `__halt_compiler()` support
/// - Optional short open tags (`<?` vs `<?php`)
...
impl<'src> Lexer<'src> {
    ...
    /// Get the current byte position in the source (used for __halt_compiler offset).
    ...
    /// Backtick strings in PHP are equivalent to shell_exec() calls.
    /// They support the same escape sequences as double-quoted strings
    /// and also support variable interpolation (expanded inline).
    /// Reference: php-src/Zend/zend_language_scanner.l
    ...
    fn scan_double_quotes_state(&mut self) -> Option<(Token, Span)> {
        ...
        loop {
            match self.peek() {
                ...
                Some('\\') => {
                    // Escape sequence — must be checked BEFORE the closing-quote check
                    // so that \" is treated as an escaped quote, not end-of-string.
                    self.consume(); // consume backslash
                    if self.peek().is_some() {
                ...
                Some('"') => {
                    // Closing quote — emit content up to (but NOT including) the `"`,
                    // then consume the `"` and switch back to InScripting.
                    // The `"` must NOT be part of the EncapsedAndWhitespace span, because
                    // `\"` (escaped quote in the middle of a string) would otherwise
                    // cause the parser's strip_suffix('"') to incorrectly remove it.
    ...
    fn scan_heredoc_state(&mut self) -> Option<(Token, Span)> {
        ...
        loop {
            match self.peek() {
                ...
                Some('{') => {
                    // Check for {$ — complex variable interpolation (same as double-quote state)
                    if self.pos + 1 < self.source.len() {
                        let next_ch = self.source.as_bytes()[self.pos + 1];
                        if next_ch == b'$' {
                            // Emit any text before the `{` as EncapsedAndWhitespace
                            ...
                            let brace_column = self.column;
                            self.consume(); // consume `{`
                            self.state_stack.push(State::Heredoc);
                            self.curly_brace_depth = 1;
                            self.state = State::InScripting;
                            return Some((
                                Token::CurlyOpen,
...
<body>
    ...
    fn test_backtick_string_simple() {
        ...
        assert_eq!(token, Token::BacktickString);
    ...
    fn test_backtick_string_with_variable() {
        ...
        // Backtick strings with variables should tokenize as BacktickString
        let (token1, _) = lexer.next_token().expect("Token 1");
        assert!(
            token1 == Token::BacktickString
                || token1 == Token::EncapsedAndWhitespace
                || token1 == Token::ConstantEncapsedString
        );
    }
...
"#;
        ...
        // Should handle as backtick string
        assert!(
            token == Token::BacktickString
                || token == Token::ConstantEncapsedString
                || token == Token::EncapsedAndWhitespace
        );
