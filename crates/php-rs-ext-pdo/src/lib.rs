...
pub enum FetchMode {
    /// Associative array keyed by column name (PDO::FETCH_ASSOC = 2).
    Assoc,
    /// Numeric array keyed by column index (PDO::FETCH_NUM = 3).
    Num,
    /// Both associative and numeric keys (PDO::FETCH_BOTH = 4, default).
    Both,
    /// Anonymous object with column names as properties (PDO::FETCH_OBJ = 5).
    Obj,
    /// Single column value (PDO::FETCH_COLUMN = 7).
    Column,
    /// Return value as key-value pair (PDO::FETCH_KEY_PAIR = 12).
    KeyPair,
    /// Group results by first column (PDO::FETCH_GROUP = 0x10000, used with fetchAll).
    Group,
    /// Only unique rows by first column (PDO::FETCH_UNIQUE = 0x30000, used with fetchAll).
    Unique,
}

/// Column metadata returned by PDOStatement::getColumnMeta().
#[derive(Debug, Clone, PartialEq)]
...
/// PDO attribute constants matching PHP values.
pub mod pdo_constants {
    // Fetch modes
    ...
    pub const FETCH_GROUP: i64 = 0x10000;
    pub const FETCH_UNIQUE: i64 = 0x30000;
...
pub trait PdoDriverStatement: Send {
    ...
    /// Return metadata for a column by index.
...
impl PdoStatement {
    ...
    /// Return metadata for a column by index.
    pub fn get_column_meta(&self, column: usize) -> Option<ColumnMeta> {
        self.inner.get_column_meta(column)
    }
}
...
pub struct PdoConnection {
    ...
    /// PDO::ATTR_ERRMODE (default: ERRMODE_EXCEPTION for php.rs)
    pub error_mode: i64,
    /// PDO::ATTR_DEFAULT_FETCH_MODE
    pub default_fetch_mode: i64,
    /// PDO::ATTR_CASE
    pub case_mode: i64,
    /// PDO::ATTR_STRINGIFY_FETCHES
    ...
    /// Driver name used for this connection.
...
impl PdoConnection {
    ...
    pub fn new(
        ...
        Ok(Self {
            conn,
            last_error: None,
            error_mode: pdo_constants::ERRMODE_EXCEPTION,
            default_fetch_mode: pdo_constants::FETCH_BOTH,
            case_mode: pdo_constants::CASE_NATURAL,
    ...
    /// Return the SQLSTATE error code for the last operation.
    pub fn error_code(&self) -> String {
        match &self.last_error {
            Some(e) => e.sqlstate.clone(),
    ...
    pub fn set_attribute(&mut self, attribute: i64, value: i64) -> bool {
        match attribute {
            pdo_constants::ATTR_ERRMODE => {
                self.error_mode = value;
                true
            }
            pdo_constants::ATTR_DEFAULT_FETCH_MODE => {
                self.default_fetch_mode = value;
                true
            }
            pdo_constants::ATTR_CASE => {
                self.case_mode = value;
                true
            }
            pdo_constants::ATTR_STRINGIFY_FETCHES => {
                self.stringify_fetches = value != 0;
                true
            }
            pdo_constants::ATTR_EMULATE_PREPARES => {
                self.emulate_prepares = value != 0;
                true
            }
            _ => false,
        }
    ...
    /// Get a PDO attribute.
    pub fn get_attribute(&self, attribute: i64) -> Option<PdoValue> {
        match attribute {
            pdo_constants::ATTR_ERRMODE => Some(PdoValue::Int(self.error_mode)),
            pdo_constants::ATTR_DEFAULT_FETCH_MODE => Some(PdoValue::Int(self.default_fetch_mode)),
            pdo_constants::ATTR_CASE => Some(PdoValue::Int(self.case_mode)),
            pdo_constants::ATTR_STRINGIFY_FETCHES => Some(PdoValue::Bool(self.stringify_fetches)),
            pdo_constants::ATTR_EMULATE_PREPARES => Some(PdoValue::Bool(self.emulate_prepares)),
            pdo_constants::ATTR_DRIVER_NAME => Some(PdoValue::Str(self.driver_name.clone())),
            pdo_constants::ATTR_SERVER_VERSION => Some(PdoValue::Str("3.0".to_string())),
            pdo_constants::ATTR_CLIENT_VERSION => Some(PdoValue::Str("3.0".to_string())),
            pdo_constants::ATTR_CONNECTION_STATUS => Some(PdoValue::Str("Connected".to_string())),
            pdo_constants::ATTR_AUTOCOMMIT => Some(PdoValue::Bool(true)),
            _ => None,
        }
    }
    ...
    pub fn get_available_drivers() -> Vec<String> {
        let mut drivers = vec!["sqlite".to_string()];
...
impl PdoDriverStatement for SqliteStatement {
    ...
    fn get_column_meta(&self, column: usize) -> Option<ColumnMeta> {
        ...
        let name = &self.column_names[column];
        let mut meta = ColumnMeta::new(name);
        meta.driver_type = "sqlite3".to_string();

        // Determine native type from first row's data
        if let Some(row) = self.rows.first() {
            if let Some(val) = row.values.get(column) {
                match val {
                    PdoValue::Null => {
                        meta.native_type = "null".to_string();
                        meta.pdo_type = 0; // PARAM_NULL
                    }
                    PdoValue::Int(_) => {
                        meta.native_type = "integer".to_string();
                        meta.pdo_type = 1; // PARAM_INT
                    }
                    PdoValue::Float(_) => {
                        meta.native_type = "double".to_string();
                        meta.pdo_type = 2; // PARAM_STR
                    }
                    PdoValue::Str(_) => {
                        meta.native_type = "string".to_string();
                        meta.pdo_type = 2; // PARAM_STR
                    }
                    PdoValue::Bool(_) => {
                        meta.native_type = "integer".to_string();
                        meta.pdo_type = 5; // PARAM_BOOL
                    }
                    PdoValue::Blob(_) => {
                        meta.native_type = "blob".to_string();
                        meta.pdo_type = 3; // PARAM_LOB
                    }
                }
            }
        }
