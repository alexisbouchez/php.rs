...
impl Superglobals {
    ...
    /// Parse a `Cookie:` header value into $_COOKIE.
    ///
    /// Cookie headers use the format: `name=value; name2=value2; ...`
    pub fn parse_cookies(&mut self, cookie_header: &str) {
        for pair in cookie_header.split(';') {
            ...
            let (key, value) = if let Some(eq) = pair.find('=') {
                (
                    ...
                    url_decode(pair[eq + 1..].trim()),
                )
            } else {
                (pair.trim().to_string(), String::new())
            };
            self.cookie.insert(key, value);
        }
    }
