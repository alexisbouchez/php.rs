...
pub fn json_encode(value: &JsonValue, options: u32) -> Option<String> {
    json_encode_with_depth(value, options, 512)
}

/// json_encode with explicit depth limit.
pub fn json_encode_with_depth(value: &JsonValue, options: u32, max_depth: usize) -> Option<String> {
    set_last_error(JsonError::None);
    let pretty = options & JSON_PRETTY_PRINT != 0;
    let unescape_slashes = options & JSON_UNESCAPED_SLASHES != 0;
    let unescape_unicode = options & JSON_UNESCAPED_UNICODE != 0;
...
fn encode_value(
    ...
    match value {
        ...
        JsonValue::Str(s) => {
            encode_string(buf, s, unescape_slashes, unescape_unicode);
        }
        ...
        JsonValue::Object(entries) => {
            ...
            for (i, (key, val)) in entries.iter().enumerate() {
                ...
                encode_string(buf, key, unescape_slashes, unescape_unicode);
                buf.push(':');
                if pretty {
                    buf.push(' ');
                }
...
fn encode_string(buf: &mut String, s: &str, unescape_slashes: bool, unescape_unicode: bool) {
    buf.push('"');
    for ch in s.chars() {
        match ch {
            '"' => buf.push_str("\\\""),
            ...
            c if !unescape_unicode && c > '\x7F' => {
                // Escape non-ASCII characters as \uXXXX (or surrogate pairs for > U+FFFF)
                let code = c as u32;
                if code <= 0xFFFF {
                    write!(buf, "\\u{:04x}", code).unwrap();
                } else {
