...
fn bench_echo_loop(c: &mut Criterion) {
    // 1M iterations of pure output generation
    let source = r#"<?php
ob_start();
for ($i = 0; $i < 1000000; $i++) {
    echo "x";
...
echo $len;
"#;
    c.bench_function("echo_loop_1M", |b| {
        b.iter(|| {
            black_box(run(source));
        });
    });

    // Pre-compiled execution only
    let oa = compile(source);
    c.bench_function("echo_loop_1M_exec_only", |b| {
        b.iter(|| {
            let mut vm = Vm::new();
            black_box(vm.execute(&oa, None).unwrap());
        });
...
echo count($filtered);
"#;
    c.bench_function("array_map_filter_sort_1000", |b| {
        b.iter(|| {
            black_box(run(source));
        });
    });

    // Array creation + access benchmark
    let source2 = r#"<?php
$arr = [];
for ($i = 0; $i < 10000; $i++) {
    $arr[$i] = $i * 2;
...
echo $sum;
"#;
    c.bench_function("array_create_access_10k", |b| {
        b.iter(|| {
            black_box(run(source2));
        });
    });

    // String-keyed array benchmark
    let source3 = r#"<?php
$arr = [];
for ($i = 0; $i < 1000; $i++) {
    $arr["key_" . $i] = $i;
...
echo $sum;
"#;
    c.bench_function("array_string_keys_1000", |b| {
        b.iter(|| {
            black_box(run(source3));
        });
    });
}
...
// ─── Copy-on-write array benchmark (13.09) ─────────────────────────────────

fn bench_array_cow(c: &mut Criterion) {
    // Benchmark array cloning followed by mutation (CoW trigger)
    let source = r#"<?php
...
echo count($original);
"#;
    c.bench_function("array_cow_clone_mutate_100", |b| {
        b.iter(|| {
            black_box(run(source));
        });
    });
